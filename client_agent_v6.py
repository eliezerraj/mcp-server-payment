...
#mcp tools url
GENERAL_SERVER_URL = "http://localhost:9000/mcp"
MATH_SERVER_URL = "http://localhost:9001/mcp"
CODE_SERVER_URL = "http://localhost:9002/mcp"
...
list_mcp_servers = MultiServerMCPClient(
    {
        "math": {
...
###########################  LLM models  #############################
def llm_model(state: AgentState, llm_model_choice, mcp_server_url) -> AgentState:
    """Specialized LLM for coding questions"""

    print("-" * 45)
    async def _run():
        async with streamablehttp_client(GENERAL_SERVER_URL) as (read, write, _):
            async with ClientSession(read, write) as session:
                await session.initialize()

                # Get tools
                mcp_tools = await list_mcp_servers.get_tools()

                agent = create_react_agent(model=llm_model_choice, tools=mcp_tools)
...
def handle_code_query(state: AgentState) -> AgentState:
    ...
    response = llm_model(state, llm_code, CODE_SERVER_URL)
...
def handle_math_query(state: AgentState) -> AgentState:
    ...
    print(f"  [MATH LLM] Code response to: {state['query']}")
    
    response = llm_model(state, llm_math, MATH_SERVER_URL)
    state['response'] = response
    return state
...
def handle_general_query(state: AgentState) -> AgentState:
    ...
    response = llm_model(state, llm_general, GENERAL_SERVER_URL)
    state['response'] = response
    return state
...
# Database Connection
try:
    DatabasePool.initialize("postgresql://postgres:postgres@localhost:5433/postgres")
    
    with DatabasePool._pool.connection() as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT 1;")
    print(f"Connect to vector database SUCCESSFULL !")
except Exception as e:
    print(f"Error connect to vector databse: {e}")
...
for i, query in enumerate(test_queries, 1):
    ...
    # Run the agent
    result = agent.invoke({'user_id':user_id ,'query': query, "topic_count": i}, thread_config)
